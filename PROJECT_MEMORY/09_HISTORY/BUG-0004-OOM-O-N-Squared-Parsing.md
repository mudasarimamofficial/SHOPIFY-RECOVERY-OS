...
## Flaw Identified
For *every single chunk*, `restore.server.ts` would invoke `await fileData.arrayBuffer()` to download the entire backup `Blob`. The `executor.ts` would then decode the full buffer into a string, invoke `.split('\n')`, and call `JSON.parse` across the entire 1,000,000-item array, only to `.slice()` 50 items and discard the rest. 
