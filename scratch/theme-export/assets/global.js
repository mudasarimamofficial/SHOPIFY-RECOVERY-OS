...
class VariantSelects extends HTMLElement {
  ...
  getAllSelectedOptions() {
    ...
    this.querySelectorAll("fieldset, .product-form__input--dropdown").forEach((group) => {
      const checked =
        group.querySelector("input:checked") || group.querySelector("select option[selected]");
      if (checked) {
  ...
  startCartLinesUpdate(items) {
    ...
    const linesByAction = Object.entries(items).reduce((groups, [variantId, quantity]) => {
      ...
      if (action === "add") {
        ...
        const lineKey = this.querySelector(`[data-quantity-variant-id="${variantId}"]`)?.dataset
          .quantityLineKey;
