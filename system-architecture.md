...
- When `Executor` creates a Product, it asks the Mapper: `mapper.get('gid://.../123')`, which substitutes `998` into the payload _before_ hitting the Shopify API adapter.
...
- _Risk Detection:_ "You are missing `read_markets` scope. 15% of your catalog will fail to restore."
- _Root Cause Analysis:_ "Restore failed on 5 products due to missing Metaobject schemas. Auto-repair suggested."
