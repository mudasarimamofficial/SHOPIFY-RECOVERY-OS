...
import { listBackups, listStores, generateRestorePlanFn, startRestoreFn, stepRestoreFn } from "@/lib/shopify.functions";
...
function RestorePage() {
  ...
  async function handleAnalyze() {
    ...
    try {
      const result = await generatePlan({ data: { backup_id: selectedBackup, target_store_id: selectedStore } });
      setPlan(result);
    } catch (e: any) {
      alert(e.message);
    } finally {
  ...
  async function handleExecute() {
    ...
    try {
      const { job_id } = await startJob({ data: { backup_id: plan.backup_id, target_store_id: plan.target_store_id, plan: plan } });
      setJobId(job_id);
          ...
          <div className="mt-6 space-y-4">
            <div>
              ...
              <select 
                ...
                value={selectedBackup} onChange={e => setSelectedBackup(e.target.value)}
                disabled={isGenerating || !!jobId}
              >
                <option value="">Select a recovery package...</option>
                {completedBackups.map(b => (
                  <option key={b.id} value={b.id}>{b.label} ({(b.stores as any)?.shop_domain})</option>
                ))}
              </select>
            </div>
            ...
            {!plan && !jobId && (
              <Button onClick={handleAnalyze} disabled={!selectedBackup || !selectedStore || isGenerating} className="w-full">
          ...
          {plan && !jobId && (
            <div className="surface-panel p-8">
              ...
              <p className="mt-2 text-sm text-muted-foreground mb-4">
                The target API version {plan.compatibility.api_versions_match ? "matches" : "does not match"} the source API.
              </p>
              <div className="divide-y divide-border border rounded-md">
                {plan.items.map((item: any, i: number) => (
                  <div key={i} className="flex justify-between p-3 text-sm">
                    <span className="font-medium">{item.resource_type} <span className="text-muted-foreground font-normal">x{item.count}</span></span>
                    <span className={item.action === "create" ? "text-success" : "text-info"}>{item.action}</span>
                  </div>
                ))}
              </div>
          ...
          {jobId && (
            <div className="surface-panel p-8 text-center">
              ...
              <div className="mt-4 h-2 w-full bg-secondary rounded-full overflow-hidden">
                <div className="h-full bg-primary transition-all duration-500" style={{ width: `${restoreProgress}%` }} />
              </div>
