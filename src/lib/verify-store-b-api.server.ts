...
export async function handleVerifyStoreBApi(request: Request) {
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      return new Response(JSON.stringify({ error: "Missing Supabase credentials in Production" }), { status: 500 });
    }
    ...
    const partnerConfig = {
      ...
      clientId: partnerClientId ? `${partnerClientId.slice(0, 4)}...${partnerClientId.slice(-4)}` : null,
    };
    ...
    for (const store of stores || []) {
      ...
      try {
        ...
        storeResults.push({
          ...
          missingScopes: scopesData.length > 0 ? missingScopes : "Could not determine (Custom App?)",
