...
export class PackageValidator {
  ...
  async validate(manifest: RecoveryManifest): Promise<boolean> {
    for (const item of manifest.catalog) {
      try {
        const rawText = await this.reader.downloadSecureStream(
          `${this.backupId}/resources/${item.type}.jsonl.enc`,
        );
...
export class RecoveryArchive {
  ...
  async writeManifest(manifest: RecoveryManifest) {
    ...
    if (!secret) {
      console.warn(
        "WARNING: RECOVERY_SIGNING_SECRET or ENCRYPTION_KEY not set. Falling back to default.",
      );
    }
    manifest.signature = createHash("sha256")
      .update(payload + (secret || "default-production-secret"))
      .digest("hex");
