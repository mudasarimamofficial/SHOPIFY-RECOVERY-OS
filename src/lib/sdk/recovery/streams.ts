...
const KMS_MASTER_KEY = process.env.ENCRYPTION_KEY ? Buffer.from(process.env.ENCRYPTION_KEY, "hex") : Buffer.alloc(32, "0"); // Production requires ENCRYPTION_KEY in hex
...
export class DecryptTransform extends Transform {
  ...
  _transform(chunk: Buffer, encoding: BufferEncoding, callback: import("stream").TransformCallback): void {
...
export class StreamingWriter {
  constructor(private admin: SupabaseClient, private bucket: string) {}
