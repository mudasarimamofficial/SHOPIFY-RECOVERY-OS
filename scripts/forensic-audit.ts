...
files.forEach(file => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('query = `{') || line.includes('query = `') || line.includes('const gql = `') || line.includes('queryStr = `{') || line.includes('queryStr: string = `{') || line.includes('mutation = `') || line.includes('createMutation = `')) {
      inQuery = true;
      currentQuery = line;
      startLine = i + 1;
